...
function sendCommand(command: BridgeCommand): Promise<unknown> {
...
function waitForActivity(lastSeen: number): Promise<unknown> {
  return new Promise((resolve, reject) => {
    if (!existsSync(SOCKET_PATH)) {
      reject(new Error(`Bridge not running (socket not found: ${SOCKET_PATH})`));
      return;
    ...
    socket.on('connect', () => {
      const command: BridgeCommand = { command: 'wait', lastSeen };
      socket.write(JSON.stringify(command) + '\n');
    });
